...
const PostDetails = () => {
  ...
  useEffect(() => {
    const fetchItemDetails = async () => {
      try {
        const response = await fetch(`https://whereisitserver.vercel.app
/lost-items/${id}`);
...
/recover-item/${id}`, {
  ...
  return (
    <div>
      <Navbar />
      <div className="w-full lg:w-8/12 mx-auto py-16 bg-gray-100 mb-10 shadow-lg">
        <h2 className="text-2xl sm:text-4xl font-bold text-center mb-6 text-black">
          {item.title}
        </h2>
        {item.thumbnail && (
          <img
        ...
        <p className="text-lg font-bold mb-4 text-black">Status: {item.postType}</p>
        ...
        {item.status === "recovered" && (
          <button className="btn bg-gray-400 text-black cursor-not-allowed " disabled>
            Already Recovered
          </button>
        )}
        ...
        <Modal open={openModal} onClose={() => setOpenModal(false)}>
          <Box sx={modalStyle}>
            ...
            <p className="mb-4">
              <strong>{item.postType === "Lost" ? "Found Description" : "Lost Description"}</strong>: {item.description}
            </p>
  
            <label className="block mb-2">{item.postType === "Lost" ? "Found Location" : "Lost Location"}</label>
            <input
              type="text"
            ...
            />
            <label className="block mb-2">{item.postType === "Lost" ? "Found Date" : "Lost Date"}</label>
            <DatePicker
        ...
        <Modal open={openSuccessModal} onClose={() => setOpenSuccessModal(false)}>
          <Box sx={successModalStyle}>
            ...
            <p className="mb-4">Your item has been successfully marked as recovered!</p>
            <button
              className="btn bg-blue-500 text-white"
